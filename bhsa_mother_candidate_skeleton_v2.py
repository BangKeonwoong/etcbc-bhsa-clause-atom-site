...
class EtcBcStyleScorer:
    ...
    def score(
        ...
        if is_rightward(daughter, mother, ctx) and not any(ev.label in RIGHTWARD_LICENSE_LABELS for ev in evidences):
            return (0.0, False, False)
...
def evaluate_generator(
    ...
    *,
    ...
    summary = {
        ...
        f"hit@{top_k}": hit_counts[f"hit@{top_k}"] / with_gold,
    ...
    for book, cnt in per_book.items():
        ...
        book_summary[book] = {
            ...
            "hit@1": cnt["hit@1"] / wg,
            "hit@3": cnt["hit@3"] / wg,
            f"hit@{top_k}": cnt[f"hit@{top_k}"] / wg,
...
def render_eval_markdown(report: Mapping[str, Any], top_k: int = 5) -> str:
    ...
    lines = [
        ...
        f"- hit@{top_k}: {summary[f'hit@{top_k}']:.4f}",
        ...
        "## Per-book",
        "",
        "| Book | with_gold | pool_cov | scored_cov | hit@1 | hit@3 | hit@%d | top1_rela_acc |" % top_k,
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for book, row in sorted(report["per_book"].items()):
        lines.append(
            f"| {book} | {row['with_gold']} | {row['candidate_pool_coverage']:.4f} | {row['scored_coverage']:.4f} | "
            f"{row['hit@1']:.4f} | {row['hit@3']:.4f} | {row[f'hit@{top_k}']:.4f} | "
            f"{row['top1_rela_accuracy_on_top1_correct']:.4f} |"
        )
...
def add_common_filter_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--resources", help="JSON file with ResourceTables")
    parser.add_argument(
        "--pool-mode",
        default="instruction",
